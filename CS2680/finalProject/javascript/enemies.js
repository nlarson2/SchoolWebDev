...
//prints this enemy
  ...
  setDifference()
  {
    this.xDif = this.xPos-playerPosX;
    this.yDif = this.yPos-playerPosY;
    if(this.xDif < 0 || this.yDif < 0)//conversts difference to a positive number if it is negative
    {
      this.xDif*=(-1);
      this.yDif*=(-1);
    }
            ...
            for(var i = 0; i<5;i++)
            {
              ...
              if(playerPosY == tempPosY-- && this.xPos == playerPosX)
              {
				...
				enemyMoving=true;//set enemy to moving to stop player from moving
		         if(this.yDif>1)//keep moving
        		    this.yPos--;
         		 if(this.yDif==1)//stop moving
        		  {
            		enemyMoving=false;
            		inBattle=true;
            		this.battling=true;
